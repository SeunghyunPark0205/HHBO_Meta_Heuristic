...
# 알고리즘 클래스 정의 (HHBO, PSO, SSA 모두 동일하게 정의)
class HHBO:
    ...
    def optimize(self):
        for t in range(self.max_iter):
            ...
            for i in range(self.num_agents):
                ...
                new_position = self.best_position + radius * np.cos(theta)  # 각 차원별로 독립적인 이동
                new_position = np.clip(new_position, self.lb, self.ub)      # 경계 값 처리
...
# 최적화 문제 설정
dim = input_size * hidden_size + hidden_size + hidden_size * output_size + output_size  # 신경망 가중치, 편향의 총 수
lb, ub = -1, 1  # 가중치 초기화 범위
