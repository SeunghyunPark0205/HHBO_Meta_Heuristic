...
for _ in range(num_experiments):
    ...
    # 최적화 실행
    _, hhbo_best_score = hhbo.optimize()
    _, pso_best_score = pso.optimize()
    _, ssa_best_score = ssa.optimize()

    # 최종 MSE 저장
    hhbo_results.append(hhbo_best_score)
    pso_results.append(pso_best_score)
    ssa_results.append(ssa_best_score)

# 평균 및 표준편차 계산
hhbo_mean, hhbo_std = np.mean(hhbo_results), np.std(hhbo_results)
pso_mean, pso_std = np.mean(pso_results), np.std(pso_results)
ssa_mean, ssa_std = np.mean(ssa_results), np.std(ssa_results)

# 결과 출력
print(f"HHBO: 평균 MSE = {hhbo_mean:.4f}, 표준편차 = {hhbo_std:.4f}")
print(f"PSO:  평균 MSE = {pso_mean:.4f}, 표준편차 = {pso_std:.4f}")
print(f"SSA:  평균 MSE = {ssa_mean:.4f}, 표준편차 = {ssa_std:.4f}")

# 결과 시각화
labels = ['HHBO', 'PSO', 'SSA']
means = [hhbo_mean, pso_mean, ssa_mean]
stds = [hhbo_std, pso_std, ssa_std]

plt.figure(figsize=(10, 6))
plt.bar(labels, means, yerr=stds, capsize=5, color=['blue', 'orange', 'green'])
plt.ylabel('Mean MSE')
plt.title('Average MSE with Standard Deviation (10 Experiments)')
plt.show()
